...
const idImpar = Pizzas.filter((pizza) => pizza.id % 2 === 1).map(
  (pizza) => pizza.nombre
);
...
console.log(
  `Las pizzas con id par son: ${idImpar
    .toString()
    .replaceAll(",", ", ")
    .replace(", Especial", " y Especial")}.`
);

const menosDe600 = Pizzas.filter((pizza) => pizza.precio < 600).map(
  (pizza) => pizza.nombre
);
...
console.log(
  `Los precios de las pizzas son: ${preciosDePizzas
    .toString()
    .replaceAll(",", ", ")
    .replace(", $1000", " y $1000")}.`
);
...
const nombresConPrecios = Pizzas.map((pizza) =>
